...
public class TriangleTestBackup1 {
	...
	public static void main(String[] args) {
		...
		TriangleTestBackup1 triTest = new TriangleTestBackup1();
		triTest.run();

		// TODO Left at drawing a triangle/drawing/rendering and presentation
	}
	...
	void run() {
		...
		// Performance measuring can be enabled, but is not good for continous drawing
		Performance.disable();
		initWindow();
		initVulkan();
		mainLoop();
	...
	PointerBuffer getRequiredInstanceExtensions(PointerBuffer glfwExtensions) {
		int numExtensions;
		if (DEBUG) {
		...
		PointerBuffer requiredExtensions = BufferUtils.createPointerBuffer(numExtensions);
		requiredExtensions.put(glfwExtensions);
		for (String baseExtensionName : REQUIRED_INSTANCE_BASE_EXTENSIONS) {
			requiredExtensions.put(MemoryUtil.memUTF8(baseExtensionName));
		}
		if (DEBUG) {
			for (String debugExtensionName : REQUIRED_INSTANCE_DEBUG_EXTENSIONS) {
				requiredExtensions.put(MemoryUtil.memUTF8(debugExtensionName));
			}
		}
	...
	void createInstance() {
		...
		appInfo.pApplicationName(stringBuffer("Hello triangle"));
		appInfo.applicationVersion(VK10.VK_MAKE_VERSION(1, 0, 0));
		appInfo.pEngineName(stringBuffer("No Engine"));
		...
		// next("createInstance createInfo2");
		createInfo.sType(VK10.VK_STRUCTURE_TYPE_INSTANCE_CREATE_INFO);
		// next("createInstance createInfo3");
		createInfo.pApplicationInfo(appInfo);
		// next("createInstance createInfo4");
		...
		int[] supportedExtensionCount = { -1 };
		validate(VK10.vkEnumerateInstanceExtensionProperties((ByteBuffer) null, supportedExtensionCount, null));
		VkExtensionProperties.Buffer supportedExtensions = VkExtensionProperties.create(supportedExtensionCount[0]);
		...
		PointerBuffer requiredExtensionsBuffer = getRequiredInstanceExtensions(requiredGLFWExtensions);
		...
		createInfo.ppEnabledLayerNames(getLayersToEnable());
	...
	PointerBuffer getLayersToEnable() {
		if (DEBUG) {
			PointerBuffer enabledLayerNames = BufferUtils.createPointerBuffer(REQUIRED_DEBUG_LAYERS.length);
			for (String requiredLayer : REQUIRED_DEBUG_LAYERS) {
				enabledLayerNames.put(MemoryUtil.memUTF8(requiredLayer, true));
			}
	...
	void setupDebugMessenger() {
		if (DEBUG) {
			...
			VkDebugUtilsMessengerCreateInfoEXT createInfo = VkDebugUtilsMessengerCreateInfoEXT.create();
			populateDebugMessengerCreateInfo(createInfo);

			long[] messengerAddress = { -1 };
			validate(EXTDebugUtils.vkCreateDebugUtilsMessengerEXT(instance, createInfo, null, messengerAddress));
			debugMessenger = messengerAddress[0];
		}
	}

	void createSurface() {
		{
			long[] surfacePointer = { -1 };
			validate(GLFWVulkan.glfwCreateWindowSurface(instance, window, null, surfacePointer));
			surface = surfacePointer[0];
		}
	}
	...
	void pickPhysicalDevice() {
		...
		{
			int[] deviceCountArray = { -1 };
			validate(VK10.vkEnumeratePhysicalDevices(instance, deviceCountArray, null));
			...
			devices = BufferUtils.createPointerBuffer(deviceCount);
			validate(VK10.vkEnumeratePhysicalDevices(instance, deviceCountArray, devices));
		}

		for (int deviceIndex = 0; deviceIndex < deviceCount; deviceIndex++) {
			long devicePointer = devices.get();
			VkPhysicalDevice currentDevice = new VkPhysicalDevice(devicePointer, instance);
			if (isSuitable(currentDevice)) {
	...
	boolean isSuitable(VkPhysicalDevice device) {
		 ...
		 */
		if (findQueueFamilies(device).isComplete() && checkExtensionSupport(device)) {
	...
	VkExtent2D chooseSwapExtent(VkSurfaceCapabilitiesKHR caps) {
		...
		// I'm not sure whether that value would be -1 or MAX_VALUE of int
		if (currentWidth != UINT_MAX) {
			return caps.currentExtent();
		} else {
			VkExtent2D actualExtent = VkExtent2D.create();
			actualExtent.width(Math.max(caps.minImageExtent().width(), Math.min(caps.maxImageExtent().width(), WIDTH)));
			actualExtent
					.height(Math.max(caps.minImageExtent().height(), Math.min(caps.maxImageExtent().height(), HEIGHT)));
			return actualExtent;
		}
	}
	...
	void createSwapchain() {
		try (MemoryStack stack = stackPush()) {
			...
			swapchainImageExtent = VkExtent2D.calloc().set(chooseSwapExtent(details.caps));
	...
	QueueFamilyIndices findQueueFamilies(VkPhysicalDevice device) {
		return findQueueFamilies(device, VK10.VK_QUEUE_GRAPHICS_BIT);
	}

	QueueFamilyIndices findQueueFamilies(VkPhysicalDevice device, int flag) {
		final int queueFamilyCount;
		final VkQueueFamilyProperties.Buffer queueFamilyProperties;
		{
			int[] queueFamilyCountPointer = { -1 };
			VK10.vkGetPhysicalDeviceQueueFamilyProperties(device, queueFamilyCountPointer, null);
			queueFamilyCount = queueFamilyCountPointer[0];
			queueFamilyProperties = VkQueueFamilyProperties.create(queueFamilyCount);
	...
	byte[] readFile(String resourceName) {
		try {
			URL url = TriangleTestBackup1.class.getClassLoader().getResource(resourceName);
	...
	void createLogicalDevice() {
		...
		VkDeviceQueueCreateInfo.Buffer queueCreateInfos = VkDeviceQueueCreateInfo
				.create(uniqueQueueFamilyIndices.size());

		{
			...
			for (Integer queueFamilyIndex : uniqueQueueFamilyIndices) {
				VkDeviceQueueCreateInfo queueCreateInfo = VkDeviceQueueCreateInfo.create();
		...
		createInfo.pEnabledFeatures(features);
		{
			...
			PointerBuffer extensionsBuffer = BufferUtils.createPointerBuffer(extensionNames.length);
			for (int index = 0; index < extensionNames.length; index++) {
				extensionsBuffer.put(index, MemoryUtil.memUTF8(extensionNames[index]));
			}
		...
		{
			PointerBuffer devicePointer = BufferUtils.createPointerBuffer(1);
			validate(VK10.vkCreateDevice(physicalDevice, createInfo, null, devicePointer));
			device = new VkDevice(devicePointer.get(), physicalDevice, createInfo);
		}
		...
		System.out.println(
				"The graphics queue has become " + graphicsQueue + " and the present queue has become " + presentQueue);
	}
	...
	ByteBuffer stringBuffer(String string) {
		ByteBuffer buffer = BufferUtils.createByteBuffer(string.length() + 1);
		for (int index = 0; index < string.length(); index++) {
			buffer.put((byte) string.charAt(index));
		}
		buffer.put((byte) 0);
		buffer.flip();
		return buffer;
	...
	void drawFrame() {
		try (MemoryStack stack = stackPush()) {
			//next("drawFrame test buffer");
			//stack.callocLong(1000);
			//next("drawFrame fences");
			{
			...
			validate(KHRSwapchain.vkAcquireNextImageKHR(device, swapchain, UINT_MAX,
			...
			validate(KHRSwapchain.vkQueuePresentKHR(presentQueue, presentInfo));
			Performance.end();
